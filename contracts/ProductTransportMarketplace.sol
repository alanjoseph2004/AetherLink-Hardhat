...
contract ProductTransportMarketplace is AccessControl {
    ...
    function completeAuction(uint256 auctionId) public {
        ...
        // --- AUTOMATIC TRANSPORT RECORD CREATION AFTER AUCTION COMPLETION ---
        if (auction.lowestBidder != address(0)) { // Check if there was a winning bid
            Product storage product = products[auction.productId];
            transportCounter++;
            ...
            transportRecords[transportCounter] = TransportRecord({
                ...
                carrier: auction.lowestBidder,
                producer: auction.producer,
                originLocation: auction.originLocation,
                destinationLocation: auction.destinationLocation,
                startTime: block.timestamp,
                estimatedDeliveryTime: block.timestamp + 1 days, // Placeholder - Consider making this dynamic
            ...
            emit TransportEvent(
                ...
                auction.productId,
                "CREATED",
                auction.lowestBidder, // Actor is the carrier who won the auction
                block.timestamp
            );
        }
        // --- END OF AUTOMATIC TRANSPORT RECORD CREATION ---
    }
